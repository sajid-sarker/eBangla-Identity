...
const TaxRecords = ({ user }) => {
  ...
  useEffect(() => {
    ...
    if (query.get("status") === "success") {
      setMessage({
        open: true,
        text: "Payment Successful! Record Updated.",
        severity: "success",
      });
  ...
  const handlePayment = async (record) => {
    try {
      ...
      const res = await axios.get(
        `${API_BASE_URL}/payment/initiate/${record._id}`,
      );
      ...
      >
        ...
        >
          <Box sx={{ backgroundColor: "#05339C", p: 3, textAlign: "center" }}>
            ...
            >
              Tax Records & Simulation
            </Typography>
            <Typography variant="body2" sx={{ color: "white", opacity: 0.8 }}>
              View official government records or simulate your own tax
            </Typography>
          </Box>
          ...
          <Box sx={{ p: 4, backgroundColor: "#fff" }}>
            ...
            >
              <Typography
                variant="h6"
                sx={{ color: "#05339C", fontWeight: "bold" }}
              >
                Official Record: {selectedYear}
              </Typography>
                  ...
                  >
                    ...
                    {records.length > 0 ? (
                      <Box sx={{ mt: 1 }}>
                        ...
                        >
                          Tax: ৳ {records[0].taxAmount.toLocaleString()}
                        </Typography>
                      ...
                      >
                        No verified record for this year.
                      </Typography>
                    )}
                  </Box>
                </Grid>
                  ...
                  >
                    ...
                    {simulatedTax ? (
                      <Box sx={{ mt: 1 }}>
                        ...
                        >
                          ৳ {simulatedTax.income.toLocaleString()}
                        </Typography>
                        <Typography
                          variant="h6"
                          sx={{ color: "text.secondary", mt: 1 }}
                        >
                          Estimated Tax: ৳ {simulatedTax.tax.toLocaleString()}
                        </Typography>
                        <Typography
                          variant="caption"
                          display="block"
                          sx={{ mt: 1, color: "gray" }}
                        >
                          * Based on 5% over 350k threshold.
                        </Typography>
                      </Box>
                    ) : (
                      <Typography
                        sx={{ fontStyle: "italic", color: "gray", mt: 1 }}
                      >
                        Enter income above to see simulation.
                      </Typography>
                    )}
                  </Box>
                </Grid>
