...
// 1. ADDED 'Grid' TO IMPORTS
import {
  Box, Typography, FormControl, InputLabel, Select, MenuItem,
  Button, Paper, TextField, CircularProgress, Divider, Alert, Snackbar, Chip, Grid
} from "@mui/material";
...
const TaxRecords = ({ user }) => {
  ...
  const handleCalculateSimulation = (e) => {
    e.preventDefault();
    const incomeValue = parseFloat(inputIncome); // Use parseFloat for accuracy
    
    if (isNaN(incomeValue) || incomeValue <= 0) {
      setMessage({ open: true, text: "Enter a valid income for simulation.", severity: "error" });
      return;
    }
    ...
    // Update local state - This makes the right-side box appear
    setSimulatedTax({ 
      income: incomeValue, 
      tax: tax 
    });
    
    setMessage({ open: true, text: "Simulation calculated successfully!", severity: "success" });
  };
      ...
      <Box component="main" sx={{ flexGrow: 1, p: { xs: 2, md: 4 }, backgroundColor: "#f9fafb" }}>
        <Paper elevation={3} sx={{ maxWidth: 900, mx: "auto", borderRadius: 4, overflow: "hidden", border: "1px solid #e5e7eb" }}>
          ...
          <Box component="form" onSubmit={handleCalculateSimulation} sx={{ p: 4, display: "flex", flexWrap: "wrap", gap: 2, justifyContent: "center", alignItems: "center", backgroundColor: "#f0f4ff" }}>
            ...
            <TextField 
              label="Expected Annual Income (BDT)" 
              type="number" 
              size="small" 
              value={inputIncome} 
              onChange={(e) => setInputIncome(e.target.value)} 
              sx={{ minWidth: 280, "& .MuiOutlinedInput-root": { borderRadius: "20px", bgcolor: 'white' } }} 
            />
            <Button 
              type="submit" 
              variant="contained" 
              startIcon={<CalculateIcon />} 
              sx={{ backgroundColor: "#05339C", borderRadius: "20px", textTransform: "none", px: 4 }}
            >
              Run Simulation
            </Button>
          </Box>
                    ...
                    <Box sx={{ border: "2px solid #05339C", borderRadius: 3, p: 3, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                        ...
                        {records.length > 0 ? (
                            <Box sx={{ mt: 1 }}>
                                ...
                                <Chip label={records[0].status} color={records[0].status === "Paid" ? "success" : "warning"} size="small" sx={{ mt: 1 }} />
                            </Box>
                        ) : (
                    ...
                    <Box sx={{ border: "2px dashed #9c27b0", borderRadius: 3, p: 3, height: '100%', bgcolor: '#fdfaff', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                        ...
                        {simulatedTax ? (
                            <Box sx={{ mt: 1 }}>
                                ...
                                <Typography variant="caption" display="block" sx={{ mt: 1, color: 'gray' }}>* Calculated based on 5% over 350k threshold.</Typography>
                            </Box>
                        ) : (
          ...
          <Box sx={{ p: 4, textAlign: "center", borderTop: "1px solid #eee" }}>
            <Button variant="contained" startIcon={<DownloadIcon />} disabled={records.length === 0}
