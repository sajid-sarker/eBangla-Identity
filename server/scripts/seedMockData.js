...
import PoliceRecord from "../src/models/PoliceRecord.js";
...
const seedData = async () => {
  try {
    ...
    if (!citizen) {
      console.error(
        "No citizen found in database. Please register a user first.",
      );
      process.exit(1);
    }
    ...
    await PoliceRecord.findOneAndUpdate({ citizen: citizen._id }, policeData, {
      upsert: true,
      new: true,
    });
