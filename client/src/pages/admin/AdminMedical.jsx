    ...
    const fetchCitizenData = async () => {
      ...
      try {
        const res = await axios.get(
          `${API_BASE_URL}/medical/admin/citizen/${selectedCitizen.nid}`,
        );
        if (res.data) {
          setFormData({
            ...
            vaccinations:
              res.data.vaccinations?.length > 0
                ? res.data.vaccinations
                : [{ name: "", hospital: "", doseNumber: 1 }],
          });
        } else {
          setFormData({
            bloodGroup: "",
    ...
    return (
      ...
      >
        ...
        <Box sx={{ flexGrow: 1, p: 4 }}>
          <Paper sx={{ p: 4 }}>
            Please select a citizen on the Home tab to manage medical records.
          </Paper>
        </Box>
      </Box>
    );
        ...
        <Box sx={{ maxWidth: "1200px", margin: "0 auto" }}>
          ...
          >
            <Typography variant="h4" sx={{ fontWeight: 800, color: "#05339C" }}>
              Manage Records: {selectedCitizen.name}
            </Typography>
            <Button
              variant="contained"
              size="large"
          ...
          >
            <Box display="flex" justifyContent="space-between" mb={2}>
              ...
              <Button
                startIcon={<AddIcon />}
                onClick={() => handleAddField("vaccinations")}
                sx={{ fontWeight: 700 }}
              >
                ADD
              </Button>
            ...
            {formData.vaccinations.map((row, i) => (
              ...
              >
                ...
                <Grid item xs={1}>
                  <IconButton
                    color="error"
                    onClick={() => removeRow("vaccinations", i)}
                  >
                    <DeleteIcon />
                  </IconButton>
                </Grid>
