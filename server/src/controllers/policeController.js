...
export const getAllUsersForPolice = async (req, res) => {
  try {
    ...
    const results = citizens.map((citizen) => {
      const record = records.find(r => r.citizen.toString() === citizen._id.toString());
      return {
        citizen,
    ...
    res.status(500).json({ message: "Server error while fetching police users." });
  }
};
...
export const uploadNidAndOcr = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Server error during file extraction processing." });
  }
};
...
export const getPoliceRecordByCitizenId = async (req, res) => {
  try {
    const { citizenId } = req.params;
    let record = await PoliceRecord.findOne({ citizen: citizenId }).populate('citizen', '-password');
    if (!record) {
    ...
    res.status(500).json({ message: "Server error fetching citizen police record." });
  }
};
...
export const addPoliceCase = async (req, res) => {
  try {
    const { citizenId } = req.params;
    const { caseNumber, crimeType, description, filedAt, station, status } = req.body;

    if (!caseNumber || !crimeType || !filedAt) {
      return res.status(400).json({ message: "caseNumber, crimeType, and filedAt are required." });
    }
    ...
    res.status(201).json({ message: "Criminal case added successfully", record });
  } catch (error) {
...
export const updatePoliceCaseStatus = async (req, res) => {
  try {
    ...
    if (!record) return res.status(404).json({ message: "Police record not found" });

    const caseIndex = record.cases.findIndex(c => c.caseNumber === caseNumber);
    if (caseIndex === -1) return res.status(404).json({ message: "Case not found" });
...
export const downloadPoliceReport = async (req, res) => {
  try {
    ...
    if (targetCitizenId && (!req.user || !req.user.isAdmin)) {
        return res.status(403).json({ message: "Not authorized to download others' reports." });
    }
    ...
    res.setHeader("Content-Disposition", `attachment; filename=Police_Report_${citizen.nid}.pdf`);

    doc.pipe(res);

    // Title Section
    doc.fillColor("#2B3C8E").fontSize(20).text("Official Police Record Report", { align: "center" });
    doc.moveDown(0.5);
    doc.fillColor("#1E1E1E").fontSize(10).text(`Generated on: ${formatDate(new Date())}`, { align: "center" });
    doc.moveDown(2);

    // Identity Details
    doc.fillColor("black").fontSize(14).font("Helvetica-Bold").text("Identity details");
    ...
    doc.moveDown(0.5);
    doc.font("Helvetica-Bold").text(`NID Verification Status : ${verificationStr.toUpperCase()}`);
    doc.font("Helvetica");
    doc.moveDown(2);
    ...
    if (!record || !record.cases || record.cases.length === 0) {
       doc.fillColor("green").text("This citizen has a completely clean police record with 0 active or historical cases.");
    } else {
       // Cases Loop
       const activeCasesCount = record.cases.filter(c => ["pending", "under_investigation", "under_trial"].includes(c.status)).length;
       doc.fillColor("black").fontSize(12).font("Helvetica");
       doc.text(`Total Registered Cases : ${record.cases.length}    Active Pending Cases: ${activeCasesCount}`);
       doc.moveDown(1.5);

       for (let i = 0; i < record.cases.length; i++) {
          const c = record.cases[i];
          doc.font("Helvetica-Bold").fontSize(11).text(`Case # ${c.caseNumber} - ${c.crimeType.toUpperCase()}`);
          doc.font("Helvetica").fontSize(10);
          doc.text(`Status : ${c.status.replace("_", " ").toUpperCase()}`);
          doc.text(`Filed on : ${formatDate(c.filedAt)} | Station : ${c.station || "N/A"}`);
