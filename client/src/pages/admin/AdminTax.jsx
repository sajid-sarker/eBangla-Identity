...
export default function AdminTax({ user }) {
  ...
  const stats = useMemo(() => {
    const totalInc = records.reduce(
      (sum, rec) => sum + (rec.totalIncome || 0),
      0,
    );
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      const res = await axios.get(
        `${API_BASE_URL}/tax/admin/citizen/${selectedCitizen._id}`,
      );
  ...
  const handleSaveFinancials = async () => {
    ...
    try {
      await axios.patch(
        `${API_BASE_URL}/tax/admin/update-citizen/${selectedCitizen._id}`,
        financials,
      );
      setSnackbar({
        open: true,
        message: "Annual income set and tax calculated!",
        severity: "success",
      });
      setOpenModal(false);
      ...
      setSnackbar({
        open: true,
        message: err.response?.data?.message || "Failed to update income",
        severity: "error",
      });
    } finally {
                        ...
                        <TableCell align="center">
                          ...
                          >
                            <Button
                              ...
                              onClick={() =>
                                handleStatusUpdate(record._id, "Paid")
                              }
                              sx={{
                                bgcolor:
                                  record.status === "Paid"
        ...
        <DialogContent>
          ...
          >
            <TextField
              ...
              onChange={(e) =>
                setFinancials({ ...financials, year: e.target.value })
              }
            >
              {years.map((year) => (
            ...
            <TextField
              ...
              onChange={(e) =>
                setFinancials({ ...financials, annualIncome: e.target.value })
              }
            />
          </Box>
        </DialogContent>
